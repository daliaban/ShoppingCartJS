...
var app = {
    ...
    showCartItems: function(){
        ...
        for (var key in busket){
            html += '<tr><td>'+i+'</td><td>'+key+'</td><td>£'+busket[key].unitprice+'</td><td>'+busket[key].quantity+'</td><td>£'
                +busket[key].price+'</td><td><button class="deleteCart">REMOVE FROM BASKET</button></td></tr>';
            i++;
        }
        html += '</tbody>';
        if (i == 1){
